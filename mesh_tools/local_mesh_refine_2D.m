function [ddom,u]=local_mesh_refine_2D(ddomold,refine_list,uold,debug,isuniform)
    ...
    if ~isuniform
        [elem_w_hanging_nodes,num_hanging,hanging_faces]=find_all_elem_w_hanging_nodes_II(ddom);
...
%         if size(ddom.mp_idx,1)<node1_idx || size(ddom.mp_idx,1)< node2_idx
%             ddom.mp_idx=[ddom.mp_idx, sparse(size(ddom.mp_idx,1),size(ddom.mp_idx,1))];
%             ddom.mp_idx=[ddom.mp_idx; sparse(size(ddom.mp_idx,1),size(ddom.mp_idx,2))];
%         end        
    ...
    function [elem_idcs,num,faces]=find_all_elem_w_hanging_nodes()
        %[elem_idcs,num,faces]=find_all_elem_w_hanging_nodes_II();
        ...
        [elem_idcs_I,num_I,faces_I]=find_all_elem_w_hanging_nodes_II();
        if any(any(elem_idcs_I~=elem_idcs)) || any(any(num_I~=num))||any(any(faces_I~=faces))
            fprintf(2,'Not the same!');
        end
    end
