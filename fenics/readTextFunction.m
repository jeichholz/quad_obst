function [u,M,elapsedtime,starttime,endtime]=readTextFunction_faster(fname)
    u=[];
    M=[];
    ...
    if deg==2
        ...
        %Add all the midpoints to the mesh.
        
        %Find all pairs vertices connected by an edge.
        %may include repeats.
        
        P=[M.C(:,1), M.C(:,2); M.C(:,2), M.C(:,3); M.C(:,1), M.C(:,3)];
        
        %uniquify
        P=sort(P,2);
        P=unique(P,'rows');
        
        %Calculate the midpoint of each pair.
        mps=1/2*(M.X(P(:,1),:)+M.X(P(:,2),:));
        
        
        numverts=size(M.X,1);
        
        %Add the midpoints to the list of vertices.
        M.X=[M.X;mps];
        
        
        %Update the global midpoint matrix.
        M.mp_idx=sparse([P(:,1); P(:,2)],[P(:,2); P(:,1)],repmat([numverts+1:numverts+size(P,1)]',2,1),size(M.X,1),size(M.X,1));
